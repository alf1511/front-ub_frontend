...
function MainPage() {
  ...
  async function getSpecificationData() {
    try {
      const resp = await axios.get('http://127.0.0.1:2124/get_spec_front');
  ...
  async function getIsInferencing() {
    try {
      const resp = await axios.get('http://127.0.0.1:2124/is_inferencing');
      const { data } = await resp;
    ...
    >
      ...
      <div className="flex flex-row justify-evenly mt-16 w-full">
        ...
        <div className="w-1/2 mr-6 border mx-3">
          <div className="overflow-y-auto w-full" style={{height : "520px"}}>
              <Table>
              ...
              {isInferencing && <LoadingInference />}
          </div>  
        </div>
        {/* TRY       */}
      </div>
